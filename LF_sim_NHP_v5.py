...
## All paths
image_path_HF = r"C:\Mount_Sinai\2_Low_Field\NHP_data\new_data\sorted_data\High_Field\NHP_DICOM\08671\Day_15"
LF_sim_saving_path = r"C:\Mount_Sinai\2_Low_Field\NHP_data\new_data\sorted_data\LF_simulated_1x1x2\input_LF_sim\08671_Day_56_slice_10"
...
# HF NHP image parameters:
# FOV: 112 x 112 x 60
# Matrix: 224 x 224 x 30
# Resolution: 0.5 x 0.5 x 2
...
if resolution_component == True:
    ...
    if dicom_input or IMA_dicom_input == True:
        ...
        # In case of NHPs, "Acquisition Matrix" key in DICOM header doesn't hold correct values. Actual values are in "Rows" and "Columns" keys.
        # Therefore, using these two keys instead of "Acquisition Matrix"
        res                    = [ds.PixelSpacing[0], ds.PixelSpacing[1], ds.SliceThickness]                         # resolution extraction from header information
...
if snr_component == True:
    ...
    # Assigning this value from phantom because NHP data is masked and has no background noise.
...
if viewing == True:
    ...
    # # Viewing single slice in a loop
    # test = LF_sim
    # for s in range(0,test.shape[2]):
    #     test_slice = test[:,:,s]
    #     fig = plt.figure()
    ...
    # fig = plt.figure()
    # plt.subplot(1, 2, 1)
    # plt.imshow(Im_HF[:,:,slice_number], cmap="gray")
    # plt.subplot(1, 2, 2)
    # plt.imshow(LF_sim[:,:,slice_number], cmap="gray")
    # plt.show()

    # # Viewing two slices in a loop
    # var_x = Im_HF
    # var_y = LF_sim
    # for s in range(0, var_y.shape[2]):
    #     var_x_slice = var_x[:, :, s]
    #     var_y_slice = var_y[:, :, s]
    #     fig = plt.figure()
    #     plt.subplot(1, 2, 1)
    #     plt.imshow(var_x_slice, cmap="gray")
    #     plt.subplot(1, 2, 2)
    #     plt.imshow(var_y_slice, cmap="gray")
    #     plt.show()
...
# for s in range(0,25):
# slice = Im_LFS_contrast_resize[:,:,3]           ## Im_HF - 11, LF_acq - 10, LF_sim - 3
# fig = plt.figure()
...
"""
####################
# Texture analysis #   ## For phantom, we don't need this
####################

## Cut out slice for choosing ROI
slice_number = 75   # check excel sheet for slice number. it varies according to brain structure
slice = LF_sim[:,:,slice_number]


## Generate ROI
##for Medulla and Frontal lobe
fig = plt.figure()
plt.imshow(slice, cmap="gray")
r = RoiPoly(color='r', fig = fig)
mask = r.get_mask(slice)
seg = slice * mask
a = np.max(seg)
b = seg.shape
...
# fig = plt.figure()
# plt.imshow(slice, cmap="gray")
# r1 = RoiPoly(color='r', fig = fig)
# fig = plt.figure()
# plt.imshow(slice, cmap="gray")
# r2 = RoiPoly(color='r', fig = fig)
# mask1 = r1.get_mask(slice)
# mask2 = r2.get_mask(slice)
# mask = mask1 + mask2
# seg = slice * mask


## Texture Analysis
features_mean, features_range, labels_mean, labels_range = glcm_features(seg)
print(features_mean)

## writing to excel file using pandas - problem with it is that it saves only one row or column - not multiple rows and columns
# df = pd.DataFrame(features_mean_glcm_lcwm)
# df.to_excel(excel_writer = "C:/Users/lenovo/Desktop/Mount Sinai/1.xlsx")
"""
